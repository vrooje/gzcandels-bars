...
Typical lower-redshift disk galaxies have minimum (i.e., edge-on) axis ratios varying from $0.08 \leq (b/a)_{min} \leq 0.2$ \citep[e.g.,][]{padilla08}, and this minimum disk thickness likely increases somewhat for disk galaxies at higher redshift \citep[e.g.,][]{forsterschreiber09,law09}. To account for the possible thickening of disks and to be conservative with respect to the maximum incompleteness correction, we assume that \emph{all} featureless galaxies with axis ratios $b/a \leq 0.4$ are disks. Assuming these are part of a randomly-oriented set of disks (\citeauthor*{lambas92} \citeyear{lambas92}; \citeauthor{binneymerrifield} \citeyear{binneymerrifield}; \citeauthor{padilla08} \citeyear{padilla08}; \citeauthor{law12b} \citeyear{law12b}) that comprise part of the \smoothlz\ sample, we determine the fraction of the \smoothlz\ sample that is composed of this hypothetical disk population. This fraction is $\approx 19\%$ for the full sample, and generally increases with redshift within our limits between 15\% and 25\%. It should be noted this is likely an overestimate of the contamination, as diskless featureless galaxies with low axis ratios certainly exist \citep{willett13}.

In order to account for this possible contamination, we add an appropriate number of featureless, not-edge-on galaxies to each redshift bin of the \disklz\ sample. The bar fractions derived below are thus conservative lower limits.
