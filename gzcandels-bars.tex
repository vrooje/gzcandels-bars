...
{\builderUC Eric F. Bell$^{X}$,}
Kevin R. V. Casteels$^{X}$, 
Christopher J. Conselice$^{X}$,
Omar Almaini $^{X}$, %emr
Henry C. Ferguson$^{ST}$,
Lucy Fortson$^{5}$, 
William Hartley$^{X}$, %emr
Dale Kocevski$^{X}$,
Anton M. Koekemoer$^{ST}$,
Daniel H. McIntosh$^{X}$,
Alice Mortlock$^{X}$, %emr
{\builderUC Jeffrey A. Newman$^{X}$,}
Jamie Ownsworth$^{X}$, %emr
%% This list is alphabetical.
...
%% This list is alphabetical.
Steven Bamford$^{X}$,
{\builderUC Tomas Dahlen$^{X}$,} %emr
{\builderUC Sandra M. Faber$^{X}$,}
{\builderUC Steven L. Finkelstein$^{X}$,} %emr
{\builderUC Adriano Fontana,$^{X}$,}
{\builderUC Audrey Galametz,$^{X}$,}
N. A. Grogin$^{ST}$,
{\builderUC Ruth Gruetzbach$^{X}$,} %emr
{\builderUC Yicheng Guo$^{X}$,}
{\builderUC Boris H\"au\ss ler$^{X}$,}
Kian Jek$^{Y}$,
Sugata Kaviraj$^{X}$,
Ray A. Lucas$^{ST}$,
{\builderUC Michael Peth$^{X}$,}
{\builderUC Mara Salvato$^{X}$,} %emr
{\builderUC Tommy Wiklind$^{X}$,} %emr
{\builderUC Stijn Wuyts$^{X}$,} %emr
%
