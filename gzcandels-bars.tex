...
\caption{
\emph{Top panel:} Bar fraction versus lookback time. Error bars are 68\% (black) and 95\% (gray) Bayesian binomial confidence intervals \citep{cameron11}; within these confidence intervals, the bar fraction is consistent with no evolution from $0.5 \leq z \leq 2$. Bins were chosen to enclose similar lookback time intervals; the bar fraction across all bins ($10^{+1.4}_{-1.1}\%$) is shown as a dashed line. Black points include correction for incompleteness described in Section \ref{sec:completeness}; purple diamonds are the uncorrected fractions. \emph{Bottom panel:} absolute $H$-band magnitudes of the featured disk sample from which the fractions are drawn.
}
\label{fig:barfrac}
\end{figure}
%%%%% END FIGURE %%%%%
